...
public class UserController {
    ...
    // TODO 智能搜索
    @SystemLog(about = "智能查询全部用户", type = LogType.DATA_CENTER,doType = "USER-15")
    @RequestMapping(value = "/getAutoCompleteNames", method = RequestMethod.POST)
    @ApiOperation(value = "智能查询全部用户")
    public Result<List<String>> getAutoCompleteNames(@RequestParam String str){
        // TODO 新增查询功能
        QueryWrapper<User> customaryUserQw = new QueryWrapper<>();
        customaryUserQw.like("nickname",str);

        List<User> userList = iUserService.list(customaryUserQw);
        List<String> userNameList = new ArrayList<>();
        userList.forEach(user -> userNameList.add(user.getNickname()));
        return new ResultUtil<List<String>>().setData(userNameList);
    }
